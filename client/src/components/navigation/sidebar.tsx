...
import { SIDEBAR_FOO_LINKS, SIDEBAR_LINKS } from "@/libs";
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <section
        className={cn(
          "bg-[#f7f8fa] dark:bg-[#1B0354]  border-r-soft-border min-[1440px]:border-l dark:border-success/40 z-[100]  transition-all duration-300 py-5 min-[900px]:flex flex-col gap-y-4 justify-between fixed lg:sticky min-h-screen left-0 top-0  border-r border-gray-200  sidebar-scroll     select-none ",
          openSidebar
      ...
      >
        <button
          ...
          onClick={() => setOpenSidebar(false)}
          className={cn(
            "absolute -right-2 transition-opacity duration-700 focus-visible:outline-2 focus-visible:outline-offset-2 w-[24px] text-light bg-white rounded-full z-[9999]",
            {
            ...
            <Link
              ...
              className={cn(
                ...
                activeLink === link.link
                  ? "[&>span]:font-semibold before:opacity-100 active dark:bg-[success/75] text-white  text-main-sec rounded outline-none"
